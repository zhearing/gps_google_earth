...
//自己加的函数和变量
public:
	void SetRowBackClr(int nrow, COLORREF clr);//设置某一行的背景颜色
	void SetAllBackClr(COLORREF clr);//设置所有行的背景色
	void SetFirstcolArrow(int iRow,int iImage);//设置某一行的第一列为选中箭头图标
	void SetCellSelected(int iRow,int iCol,int bListMod=FALSE);//设置某一个单元格被选中状态
	void SetAllcolNoArrow(void);//设置所有行没有箭头
	void DeleteOneRow(int iRow);//删除指定的一行数据
	CString m_strNumSort;//按数字排列的字符串标识
	void SetColumnSortByNum(int iCol);//设置某一列按数字排序
	bool m_bFirstColArrow;//第一列显示箭头的标志变量，当为真时，当前选中的行的第一列有黑色箭头
	int SelectBySQLInitGrid(CString strSQL,bool bIsClear=true);//根据SQL语句查询数据库，并用查询的结果初始化表格
	BOOL DeleteBySQLInitGrid(CString strSQL,int iRow);//根据SQL语句删除数据库一条记录，并用删除后的结果初始化表格
	void SetColCheck(int iStartRow,int iCol);//设置某一列为复选按钮
	int FindstrInGrid(CString str,int iCol);//根据某一列查询是否有匹配字符串
	float AccountSumByCol(int iCol);//求某数值列的和
};
