...
void CGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
    ...
    if (next != m_idCurrentCell)
    {
        ...
        if (m_MouseMode == MOUSE_NOTHING)
        {
			...
			//自己加
			if(m_bFirstColArrow==TRUE)
			{
				SetAllcolNoArrow();
				SetFirstcolArrow(next.row,0);
				Refresh();
			}
			//自己加
            m_MouseMode = MOUSE_NOTHING;
...
BOOL CGridCtrl::SortItems(int nCol, BOOL bAscending, LPARAM data /* = 0 */)
{
	...
	if(nCol<=(m_strNumSort.GetLength()-1))
	{
		if(m_strNumSort.GetAt(nCol)=='1')
...
void CGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
...
#endif
    {
		...
		if(m_bFirstColArrow==TRUE)
		{
			...
			{
				SetAllcolNoArrow();
				SetFirstcolArrow(m_LeftClickDownCell.row,0);
				Refresh();
			}
		}
		//自己加
        ...
        if (m_LeftClickDownCell.row < GetFixedRowCount())
		{
			...
			if(GetRowCount()>1)
			{
				SetCellSelected(1,1,FALSE);
				if(m_bFirstColArrow==TRUE)
				{
					SetAllcolNoArrow();
					SetFirstcolArrow(1,0);
					Refresh();
				}
			}
			//自己加
        ...
        else if (m_LeftClickDownCell.col < GetFixedColumnCount())
		{
			...
			//自己加
			if(GetRowCount()>1)
				SetCellSelected(m_LeftClickDownCell.row,1,TRUE);
			//自己加
		}
        else
        {
...
void CGridCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
    ...
    else
    {
		...
		{
			SetAllcolNoArrow();
			SetFirstcolArrow(m_idCurrentCell.row,0);
			Refresh();
		}
		//自己加
        CGridCellBase* pCell = GetCell(m_idCurrentCell.row, m_idCurrentCell.col);
...
void CGridCtrl::OnRButtonUp(UINT nFlags, CPoint point)
{
    ...
    else
    {
		...
		if(GetRowCount()>1)
		{
			if(FocusCell.row>0)
			{
				if(FocusCell.col < GetFixedColumnCount())
					SetCellSelected(FocusCell.row,FocusCell.col,TRUE);
				else
					SetCellSelected(FocusCell.row,FocusCell.col,FALSE);
				if(m_bFirstColArrow==TRUE)
				{
					SetAllcolNoArrow();
					SetFirstcolArrow(FocusCell.row,0);
					Refresh();
				}
			}
			else
			{
				SetCellSelected(1,FocusCell.col,FALSE);
				if(m_bFirstColArrow==TRUE)
				{
					SetAllcolNoArrow();
					SetFirstcolArrow(1,0);
...
//设置某一行的背景颜色，自己加的函数
void CGridCtrl::SetRowBackClr(int nrow, COLORREF clr)
{
	int nFixeColCount=GetFixedColumnCount();
	int nColCount=GetColumnCount();
	for(int i=nFixeColCount;i<nColCount;i++)
	{
		GetCell(nrow,i)->SetBackClr(clr);
	}
	Invalidate();
}
//设置所有行的背景颜色，自己加的函数
void CGridCtrl::SetAllBackClr(COLORREF clr)
{
 	int nFixeColCount=GetFixedColumnCount();
 	int nColCount=GetColumnCount();
 	int nFixeRowCount=GetFixedRowCount();
 	int nRowCount=GetRowCount();
 	int i,j;
 	for(i=nFixeRowCount;i<nRowCount;i++)
 	{
 		for(j=nFixeColCount;j<nColCount;j++)
 		{
 			GetCell(i,j)->SetBackClr(clr);
 		}
 	}
 	Invalidate();
}
//设置某一行的第一列为选中箭头图标,iRow为要添加图标的行,iImage=0为箭头图标,iImage=1为空图标
void CGridCtrl::SetFirstcolArrow(int iRow,int iImage)
{
	GV_ITEM Item;
	Item.col=0;	
	Item.row=iRow;
...
//设置某一单元格为选中状态
void CGridCtrl::SetCellSelected(int iRow,int iCol,BOOL bListMode)
{
	CCellID cell(iRow,iCol);
	SetFocusCell(cell);
	if(bListMode==FALSE)
		SetSelectedRange(iRow,iCol,iRow,iCol, TRUE);
	else
		SetSelectedRange(iRow,GetFixedColumnCount(), iRow,GetColumnCount()-1, TRUE);
}
//设置所有行没有箭头
void CGridCtrl::SetAllcolNoArrow(void)
{
	int iRowCount=GetRowCount();
	for(int i=1;i<iRowCount;i++)
		SetFirstcolArrow(i,1);
}
//删除指定的一行数据
void CGridCtrl::DeleteOneRow(int iRow)
{
	int iRowCount=GetRowCount();
	if(iRow>=1)
	{
...
//		ExpandLastColumn();
		if(iRow==iRowCount-1)
		{
			...
			{
				SetCellSelected(iRow-1,1,FALSE);
				if(m_bFirstColArrow==TRUE)
					SetFirstcolArrow(iRow-1,0);
			}
		}
		else
		{
			SetCellSelected(iRow,1,FALSE);
			if(m_bFirstColArrow==TRUE)
				SetFirstcolArrow(iRow,0);
		}
	}
}
//设置某一列按数字排序1,2,3,4.....
void CGridCtrl::SetColumnSortByNum(int iCol)
{
	int iRowCount=GetRowCount();
	if(iRowCount<=1)
		return;
	...
	else if(iRowCount<100)
	{
		for(int i=1;i<iRowCount;i++)
		{
			Item.row=i;
			if(i<10)
				Item.strText.Format("0%d",i);
			else
				Item.strText.Format("%d",i);
			SetItem(&Item);
		}
	}
	else if(iRowCount<1000)
	{
		for(int i=1;i<iRowCount;i++)
		{
			Item.row=i;
			if(i<10)
				Item.strText.Format("00%d",i);
			else if(i<100)
				Item.strText.Format("0%d",i);
			else
				Item.strText.Format("%d",i);
	...
	else if(iRowCount<10000)
	{
		for(int i=1;i<iRowCount;i++)
		{
			Item.row=i;
			if(i<10)
				Item.strText.Format("000%d",i);
			else if(i<100)
				Item.strText.Format("00%d",i);
			else if(i<1000)
				Item.strText.Format("0%d",i);
...
//根据SQL语句查询数据库，并用查询的结果初始化表格bIsClear=true表示清空列表原来数据，bIsClear=false表示不清空列表原来数据，
int CGridCtrl::SelectBySQLInitGrid(CString strSQL,bool bIsClear)
{
	CAdoConnection con;
	CAdoRecordSet res;
	if (con.ConnectAccess("db.mdb"))
	{
		...
		for(int i=0;i<iRowCount;i++)
		{
			...
			for(int j=0;j<iColCount;j++)
			{
				Item.col=j+1;
				Item.row=i+1;
				res.GetCollect((long)j,value);
				if(adDouble==res.GetFieldType(j))
				{
					value=value.Left(value.GetLength()-6);
				}
				Item.strText=value;
				SetItem(&Item);
			}
...
//根据SQL语句删除数据库一条记录，并用删除后的结果初始化表格
BOOL CGridCtrl::DeleteBySQLInitGrid(CString strSQL,int iRow)
{
	CAdoConnection con;
	CAdoRecordSet res;
	if (con.ConnectAccess("database.mdb"))
	{
		res.SetAdoConnection(&con);
		if(res.Open(strSQL))
		{
...
//设置某一列为复选按钮
void CGridCtrl::SetColCheck(int iStartRow,int iCol)
{
	int iRowCount=GetRowCount();
	for(int i=iStartRow;i<iRowCount;i++)
	{
		SetCellType(i,iCol, RUNTIME_CLASS(CGridCellCheck));
	}
}
//根据某一列查询是否有匹配字符串,没找到返回-1，否则返回找到的字符串所在的行数
int CGridCtrl::FindstrInGrid(CString str,int iCol)
{
	int iRowCount=GetRowCount();
	int iFixedRowCount=GetFixedRowCount();
	int iDataRowCount=iRowCount-iFixedRowCount;
	for(int i=iFixedRowCount;i<=iDataRowCount;i++)
	{
		if(str==GetItemText(i,iCol))
			return i;
	}
	return -1;
}
//求某数值列的和
float CGridCtrl::AccountSumByCol(int iCol)
{
	int iRowCount=GetRowCount();
	int iFixedRowCount=GetFixedRowCount();
	int iDataRowCount=iRowCount-iFixedRowCount;
